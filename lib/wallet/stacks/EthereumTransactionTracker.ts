...
class EthereumTransactionTracker {
  ...
  public init = async (): Promise<void> => {
    ...
    this.logger.info(`Starting Rsk transaction tracker for address: ${this.walletAddress}`);
    ...
    for (const transaction of block.transactions) {
      if (transaction.from === this.walletAddress) {
        ...
        if (confirmedTransactions.length > 0) {
          this.logger.debug(`Removing ${confirmedTransactions.length} confirmed Rsk transactions`);

          for (const confirmedTransaction of confirmedTransactions) {
            this.logger.silly(`Removing confirmed Rsk transaction: ${confirmedTransaction.hash}`);
