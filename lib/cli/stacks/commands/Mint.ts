...
export const command = 'mint <preimage> [token]';

// or a ERC20 token
export const describe = 'mints Stx from the corresponding swap contract';
...
export const handler = async (argv: Arguments<any>): Promise<void> => {
  ...
  contractAddress = nftAddress.split('.')[0];
  contractName = nftAddress.split('.')[1];
  privkey = allargs[4];
  customfee = 90000;
  // if (allargs[9]) {
