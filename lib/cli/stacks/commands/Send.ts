...
import { connectEthereum, getBoltzAddress, getContracts } from '../StacksUtils';

export const command = 'send <amount> [destination] [token]';

export const describe = 'sends Rbtc or a ERC20 token to the destined address';
...
export const handler = async (argv: Arguments<any>): Promise<void> => {
  ...
  console.log(`Sent ${argv.token ? 'ERC20 token' : 'Rbtc'} in: ${transactionHash}`);
};
