...
const functionName = 'oracle-instant-helper';
...
class AlexOracle implements Exchange {
  public async getPrice(baseAsset: string, quoteAsset: string): Promise<number> {
    ...
    try {
      ...
      console.log('AlexOracle.62 returning ', baseAsset, quoteAsset, cvToJSON(result).value.value);
      return parseInt(cvToJSON(result).value.value)/10**8;
    } catch(error) {
      throw 'Alex Oracle failure';
    }
  }
