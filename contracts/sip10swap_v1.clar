...
;; Built as part of https://github.com/stacksgov/Stacks-Grants/issues/172 to enable swaps SIP10 tokens <-> BTC on Lightning Network 
...
(define-map swaps {hash: (buff 32)} {locked: bool, initiator: principal, claimPrincipal: principal, tokenPrincipal: principal})
...
(define-public (lockStx (preimageHash (buff 32)) (amount (buff 16)) (tokenAddress (buff 42)) (claimAddress (buff 42)) (timelock (buff 16)) (claimPrincipal principal) (tokenPrincipal <ft-trait>))
  ...
  (begin
    (asserts! (> txamount u0) (err false))
    (asserts! (is-eq (checkSwapIsLocked calculatedHash) false) (err false))
...
;; @param preimageHash Preimage hash of the swap
;; @param amount Amount the swap has locked in msip10
;; @param timelock Block height after which the locked sip10 can be refunded
...
;; true: swap with hash has sip10 locked
