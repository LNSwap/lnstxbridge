...
// , getHexString, getUnixTime decodeInvoice, reverseBuffer
...
const mockGetChannelCreationResult: any = undefined;
...
const mockGetRawTransactionResult = '';
...
// const MockedStacksClient = <jest.Mock<StacksClient>><any>StacksClient;
...
describe('SwapManager', () => {
  ...
  test('it should init', async() => {
    ...
    expect(manager.currencies.get('STX')).toEqual(ltcCurrency);
  ...
  //   // baseCurrency
  //   expect(mockGetRoutingHints).toHaveBeenCalledWith(quoteCurrency, nodePublicKey);
  ...
  // need to fix lndclient! - should work after removing custodial nft mint
  ...
  test('it should get currencies', () => {
    ...
    expect(getCurrencies('STX', 'BTC', OrderSide.BUY)).toEqual({
      sendingCurrency: {
        ...ltcCurrency,
        wallet: mockWallets.get('STX'),
      },
      receivingCurrency: {
        ...btcCurrency,
        wallet: mockWallets.get('BTC'),
    ...
    expect(getCurrencies('STX', 'BTC', OrderSide.SELL)).toEqual({
      sendingCurrency: {
        ...btcCurrency,
        wallet: mockWallets.get('BTC'),
      },
